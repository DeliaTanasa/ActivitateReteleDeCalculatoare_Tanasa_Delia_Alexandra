...
class FTPClient:
    ...
    def get_server_files(self):
        """Helper: get list of files from server and let user pick one"""
    ...
    def read_file(self):
        ...
        filename = self.get_server_files()
        if not filename:
            return
    ...
    def download(self):
        ...
        if response['status'] == 'error':
            ...
            downloaded_filename = response.get('filename', filename)
            
            filepath = os.path.join(LOCAL_FILES_DIR, downloaded_filename)
            try:
                with open(filepath, 'w') as f:
                    f.write(content)
                print(f"✓ File '{downloaded_filename}' saved to {LOCAL_FILES_DIR}/")
            except Exception as e:
    ...
    def edit_file(self):
        ...
        read_response = self.send_command(read_command)
        
        if read_response['status'] == 'success':
            print(f"\nCurrent content of '{filename}':")
            print("=" * 40)
            print(read_response.get('content', ''))
            print("=" * 40)
    ...
    def see_file_operation_history(self):
        ...
        if response['status'] == 'error':
            ...
            if not history:
                print("  No operations recorded for this file.")
            else:
                print("=" * 50)
